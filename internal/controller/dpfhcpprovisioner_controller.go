...
func (r *DPFHCPProvisionerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Generate ignition configuration for DPF provisioning
	// Only runs after HostedCluster creation (hostedClusterRef is set) and if IgnitionGenerating phase
	if cr.Status.HostedClusterRef != nil && cr.Status.Phase == provisioningv1alpha1.PhaseIgnitionGenerating {
		...
		if result, err := r.IgnitionGenerator.ReconcileIgnition(ctx, &cr); err != nil || result.RequeueAfter > 0 {
			if err != nil {
		...
		log.V(1).Info("Skipping ignition generation - HostedCluster not ready", "phase", cr.Status.Phase)
	}

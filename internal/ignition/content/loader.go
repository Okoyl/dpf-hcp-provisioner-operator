...
import (
	...
	"github.com/rh-ecosystem-edge/dpf-hcp-provisioner-operator/internal/ignition/util"
)
...
func AddFiles(ign *ignition.Ignition, provider ContentProvider) error {
	...
	for _, fileDef := range files {
		...
		case []byte:
			...
			contents = ignition.EncodeGzipFile(source)
		default:
			return fmt.Errorf("invalid content source type for %s", fileDef.Path)
		}

		// Convert mode from octal if needed
		mode := fileDef.Mode
		if mode >= 400 && mode < 1000 {
			mode = util.OctalToDecimal(mode)
		}
